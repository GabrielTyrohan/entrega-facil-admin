// ============================================
// ARQUIVO: src/pages/AcertosDiarios/NovoAcerto.tsx
// SUBSTITUIR TODO O ARQUIVO POR ESTA VERSÃO
// ============================================
...
const CurrencyInput = ({ label, field, value, onChange }: { label: string, field: string, value: number, onChange: (field: string, val: string) => void }) => (
  <div>
...
const NovoAcerto = () => {
  ...
  // ===== CÁLCULOS CORRETOS (BASEADO NO PROCESSO REAL) =====

  // 1. TOTAL RECEBIDO (dinheiro que vendedor entregou)
  ...
  // 2. TOTAL DESPESAS (comprovantes que ABATER do valor que ele deve)
  ...
  // 3. TOTAL VENDAS (quanto o vendedor DEVE ao caixa)
  const totalVendas = useMemo(() => 
    vendas.reduce((sum, venda) => sum + venda.valor, 0),
    [vendas]
  );

  // 4. VALOR ESPERADO (quanto ele DEVERIA entregar)
  const valorEsperado = useMemo(() => 
    totalVendas - totalDespesas,
    [totalVendas, totalDespesas]
  );

  // 5. SALDO (diferença entre o que entregou e o esperado)
  // POSITIVO = Vendedor entregou A MAIS (sobra)
  // NEGATIVO = Vendedor entregou A MENOS (falta)
  const saldoLiquido = useMemo(() => 
    totalRecebido - valorEsperado,
    [totalRecebido, valorEsperado]
  );
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
                ...
                {vendas.map((venda, index) => (
                  <div key={venda.id} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-right bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm"
                      placeholder="R$ 0,00"
                    />
                    {vendas.length > 1 && (
                      <button
      ...
      {/* Modal de Ajuda - CONTINUA NO PRÓXIMO COMENTÁRIO... */}
      <Modal
        isOpen={showHelp}
        ...
        <div className="space-y-4 text-gray-600 dark:text-gray-300">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
              ...
              Soma de tudo que o vendedor entregou (dinheiro, comprovantes de PIX/depósito, etc).
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-3 text-xs">
              <div className="p-2 bg-green-50 dark:bg-green-900/10 rounded border-l-4 border-green-500">
                <div className="font-semibold text-green-800 dark:text-green-300 mb-1">✅ Cenário: SOBRA (Saldo Positivo)</div>
                <div className="space-y-0.5 text-gray-700 dark:text-gray-400">
                  <div>Vendas: R$ 500,00</div>
                  <div>Despesas: R$ 50,00 (gasolina)</div>
                  <div>Esperado: R$ 450,00 (500 - 50)</div>
                  <div>Recebido: R$ 480,00</div>
                  <div className="font-semibold text-green-700 dark:text-green-400 mt-1">SOBRA: R$ 30,00 (vendedor entregou a mais)</div>
                </div>
              </div>

              <div className="p-2 bg-red-50 dark:bg-red-900/10 rounded border-l-4 border-red-500">
                <div className="font-semibold text-red-800 dark:text-red-300 mb-1">❌ Cenário: FALTA (Saldo Negativo)</div>
                <div className="space-y-0.5 text-gray-700 dark:text-gray-400">
                  <div>Vendas: R$ 500,00</div>
                  <div>Despesas: R$ 50,00</div>
                  <div>Esperado: R$ 450,00</div>
                  <div>Recebido: R$ 420,00</div>
                  <div className="font-semibold text-red-700 dark:text-red-400 mt-1">FALTA: R$ 30,00 (vendedor deve pagar)</div>
                </div>
              </div>
            </div>
          </div>
